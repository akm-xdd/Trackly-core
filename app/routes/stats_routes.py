...
def get_daily_stats(
    ...
    current_user: UserResponse = Depends(
        require_maintainer_or_admin)  # Keep admin-only
):
    """Get daily statistics (MAINTAINER+ only)"""
    return StatsService.get_all_daily_stats(db, limit=limit)
...
def get_stats_summary(
    ...
    if today_stats and yesterday_stats:
        changes = {
            "total_change": today_stats.total_issues -
            yesterday_stats.total_issues,
            "open_change": today_stats.status_open -
            yesterday_stats.status_open,
            "critical_change": today_stats.severity_critical -
            yesterday_stats.severity_critical}
