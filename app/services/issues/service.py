...
class IssueService:
   ...
   def create_issue(db: Session, issue_data: IssueCreate, created_by: str) -> IssueResponse:
       try:
           ...
           creator = db.query(UserSchema).filter(UserSchema.id == created_by).first()
   ...
   def update_issue(db: Session, issue_id: str, issue_data: IssueUpdate, updated_by: str) -> Optional[IssueResponse]:
       ...
       try:
           ...
           creator = db.query(UserSchema).filter(UserSchema.id == db_issue.created_by).first()
           updater = db.query(UserSchema).filter(UserSchema.id == updated_by).first()
   ...
   def delete_issue(db: Session, issue_id: str, deleted_by: str = None) -> bool:
       ...
       try:
           ...
           if deleted_by:
               deleter = db.query(UserSchema).filter(UserSchema.id == deleted_by).first()
               asyncio.create_task(
                   broadcaster.broadcast_issue_event(
                       IssueEvent(
