...
class StatsService:
    ...
    def aggregate_daily_stats(target_date: date = None) -> dict:
        ...
        try:
            ...
            status_results = (
                db.query(
                    IssueSchema.status, func.count(
                        IssueSchema.id)) .filter(
                    func.date(
                        IssueSchema.created_at) <= target_date) .group_by(
                    IssueSchema.status) .all())

            severity_results = (
                db.query(
                    IssueSchema.severity, func.count(
                        IssueSchema.id)) .filter(
                    func.date(
                        IssueSchema.created_at) <= target_date) .group_by(
                    IssueSchema.severity) .all())
            ...
            logger.info(
                f"Successfully aggregated daily stats for {target_date}: {total_issues} total issues")
            ...
            logger.error(
                f"Error aggregating daily stats for {target_date}: {str(e)}")
            raise e
        finally:
            db.close()
    ...
    def save_daily_stats(
            db: Session,
            stats_data: DailyStatsCreate) -> Optional[DailyStatsResponse]:
        ...
        try:
            ...
            raise HTTPException(status_code=500,
                                detail=f"Failed to save daily stats: {str(e)}")

    @staticmethod
    def get_daily_stats(
            db: Session,
            target_date: date) -> Optional[DailyStatsResponse]:
    ...
    def get_all_daily_stats(
            db: Session,
            limit: int = 30) -> List[DailyStatsResponse]:
